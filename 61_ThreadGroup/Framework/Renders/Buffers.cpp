...
{
	functionList.push_back(bind(&CsResource::CreateInput, this));
	functionList.push_back(bind(&CsResource::CreateSRV, this));
	functionList.push_back(bind(&CsResource::CreateOutput, this));
	functionList.push_back(bind(&CsResource::CreateUAV, this));
	functionList.push_back(bind(&CsResource::CreateResult, this));
...
RawBuffer::RawBuffer(void* input, UINT inputByte, UINT outputByte)
: CsResource(), inputData(input), inputByte(inputByte), outputByte(outputByte)
{
	CreateBuffer();
}
...
void RawBuffer::CreateInput()
{
	...
	// raw buffer(shader에서는 byte Address)에서 받을 수 있는 버퍼 용도 명시
	...
	Check(D3D::GetDevice()->CreateBuffer
	(
	&desc,
	inputData == nullptr ? nullptr : &subResource,
	&buffer
	)
	);
...
void RawBuffer::CreateSRV()
{
	...
	// 4byte 자료형 하나. 타입은 모르기 때문에 TYPELESS
...
void RawBuffer::CopyFromOutput(void* data)
{
	...
	// USAGE에 관계 없이 output을 result에 복사.
