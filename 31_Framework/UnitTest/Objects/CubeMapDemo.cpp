...
//	meshCube->Scale(scale);
	...
	cubeMap->RotationDegree(cubemapRot);
#endif
}
...
void CubeMapDemo::CreateMesh()
{
	...
	for (size_t i = 0; i < 5; i++)
	{
		...
		meshCylinder[2 * i]->Position(-30, 6, -15 * (float)i - 15.0f);
		meshCylinder[2 * i]->Scale(5, 5, 5);
		meshCylinder[2 * i] -> DiffuseMap(L"Bricks.png");


		meshCylinder[2 * i + 1] = new MeshCylinder(shader, 0.5f, 3.0f, 20, 20);
		meshCylinder[2 * i + 1]->Position(30, 6, -15 * (float)i - 15.0f);
		meshCylinder[2 * i + 1]->Scale(5, 5, 5);
	...
	for (size_t i = 0; i < 5; i++)
	{
		...
		meshSphere[2 * i] -> Position(-30, 15.5, -15 * (float)i - 15.0f);
		meshSphere[2 * i]->Scale(5, 5, 5);
		meshSphere[2 * i]->DiffuseMap(L"Wall.png");



		meshSphere[2 * i + 1] = new MeshSphere(shader, 0.5f);
		meshSphere[2 * i + 1]->Position(30, 15.5, -15 * (float)i - 15.0f);
		meshSphere[2 * i + 1]->Scale(5, 5, 5);
