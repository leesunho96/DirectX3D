...
void RayCastDemo::Update()
{
	...
	DebugLine::Get()->RenderLine(position.x, position.y - 1.0f, position.z, position.x, 50, position.z);

	DebugLine::Get()->RenderLine(position.x, position.y - 1.0f, position.z, position.x, 0.0f, position.z, Color(1, 0, 0, 1));
	

	Context::Get()->GetCamera()->Forward();
